...
def run_one(df):
    ...
    Xtr_txt, Xte_txt, ytr, yte = train_test_split(
        df["text"].values, y, test_size=0.2, stratify=y, random_state=SEED
    )
    if yte.sum() == 0:
        return None
    ...
    for name, clf in [("NB", MultinomialNB()),
        ...
        order_ = np.argsort(-sc)
        f95_ = screen_fraction_for_recall(yte[order_], 0.95)
        if best is None or (f95_ is not None and f95_ < best[1]):
            best = (name, f95_, sc)
    model_name, _, scores = best
...
def main():
    ...
    for r in rows:
        print(f"{r['name'][:33]:<34}{r['n_test']:>7}{r['pos_test']:>5}{r['prevalence']*100:>6.1f}%"
              f"{r['screen_for_95']*100:>11.1f}%{r['screen_for_100']*100:>12.1f}%{r['wss95']*100:>8.1f}%  {r['model']}")
    print("-" * 90)
